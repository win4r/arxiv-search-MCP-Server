 ...
 * 缓存最近的搜索结果
 */
const recentPapers: { [id: string]: Paper } = {};
...
async function parseArxivResponse(xmlData: string): Promise<Paper[]> {
  ...
  try {
    ...
    return entries.map((entry: any): Paper => {
      ...
      // 获取PDF链接
      const pdfLink = entry.link.find((link: any) => link.$.title === 'pdf')?.$.href || '';
 ...
 * 搜索arXiv论文
 */
async function searchArxiv(query: string, maxResults: number = 10, sortBy: string = 'relevance', start: number = 0): Promise<Paper[]> {
  try {
